"""Major League Baseball section."""
from __future__ import annotations
import csv, io
from . import mlb_teams_data as DATA
from . import socials

SOCIAL_FIELDS = socials.SOCIAL_FIELDS
INFO = {"label": "Major League Baseball", "edition": DATA.MLB_EDITION, "url": DATA.MLB_SOURCE_URL, "note": "All 30 MLB teams by league and division."}
...
def get_rows(live: bool = False):
    rows = []
    for r in DATA.MLB_TEAMS:
        website = _clean(r.get("website"))
        item = {"rank": r.get("rank"), "website": website, "website_display": _display(website)}
        for _lbl, f in [('Team', 'team'), ('League', 'league'), ('Division', 'division'), ('City', 'city'), ('Stadium', 'stadium')]:
...
def columns():
    cols = [("Rank", "rank")] + [('Team', 'team'), ('League', 'league'), ('Division', 'division'), ('City', 'city'), ('Stadium', 'stadium')] + [("Website", "website")]
