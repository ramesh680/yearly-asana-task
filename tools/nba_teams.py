"""National Basketball Association section."""
from __future__ import annotations
import csv, io
from . import nba_teams_data as DATA
from . import socials

SOCIAL_FIELDS = socials.SOCIAL_FIELDS
INFO = {"label": "National Basketball Association", "edition": DATA.NBA_EDITION, "url": DATA.NBA_SOURCE_URL, "note": "All 30 NBA teams by conference and division."}
...
def get_rows(live: bool = False):
    rows = []
    for r in DATA.NBA_TEAMS:
        website = _clean(r.get("website"))
        item = {"rank": r.get("rank"), "website": website, "website_display": _display(website)}
        for _lbl, f in [('Team', 'team'), ('Conf', 'conference'), ('Division', 'division'), ('City', 'city'), ('Arena', 'arena')]:
...
def columns():
    cols = [("Rank", "rank")] + [('Team', 'team'), ('Conf', 'conference'), ('Division', 'division'), ('City', 'city'), ('Arena', 'arena')] + [("Website", "website")]
